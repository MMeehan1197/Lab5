...
public class RootLayoutController implements Initializable {
	...
	public void BuildMenus()
	{
		...
		Menu mnuPot = new Menu();
		mnuGame.setText("Pick Game");
		mnuPot.setText("Pot Rules");
		mb.getMenus().add(0,mnuGame);
		mb.getMenus().add(0,mnuPot);
		...
		for (ePotRule ePot : ePotRule.values()) {
			RadioMenuItem rmi = new RadioMenuItem(ePot.toString());
			rmi.setId("PokerPot" + String.valueOf(ePot.getPot()));
			rmi.setToggleGroup(tglGrpPot);
			if (ePot.getDefault())
			{
				rmi.setSelected(true);
			}
			mnuPot.getItems().add(rmi);
		}
